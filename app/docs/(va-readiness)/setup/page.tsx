  ...
  return (
    <div>
      ...
      <section className="mb-10">
        <div className="relative aspect-video w-full overflow-hidden rounded-xl border  border-primary-blue-100/70 bg-muted shadow-sm">
          <iframe
      ...
      <section className="space-y-6">
          ...
          <div className="space-y-2.5">
            {accountItems.map((item) => (
              <label
                key={item.id}
                htmlFor={item.id}
                className="flex items-start gap-3 rounded-lg border border-primary-blue-100/70 p-3.5 cursor-pointer transition-colors hover:bg-primary-blue-100/30 has-[button[data-state=checked]]:bg-primary-blue-100/20"
              >
                <Checkbox
                  id={item.id}
                  ...
                  className="mt-0.5 shrink-0  border-primary-blue-100/70 data-checked:bg-primary-blue-100"
                />
                <div>
                  <p
                    className={`text-sm text-white font-medium transition-colors ${checked[item.id] ? "line-through text-white/70" : "text-white"}`}
                  >
                    {item.label}
                  </p>
        ...
        <div className="mt-8">
          ...
          <div className="rounded-lg border border-primary-blue-100/70 divide-y divide-primary-blue-100/70">
            {[
              {
                field: "Target Niche",
