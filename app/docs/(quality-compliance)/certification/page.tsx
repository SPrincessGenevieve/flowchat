...
export default function CertificationPage() {
  ...
  return (
    <div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 mb-8">
        <div>
          ...
          <input
            ...
            className="w-full rounded-[5px] border bg-primary-blue-100/20 border-primary-blue-100 px-3 py-2 text-sm text-white placeholder:text-white/70 focus:outline-none focus:ring-2 focus:ring-ring"
          />
        </div>
        <div>
          <label className="block text-xs font-semibold uppercase tracking-widest text-white/70 mb-1.5">
      ...
      <div
        className={cn(
          "mb-8 rounded-[5px] border p-4 flex items-center justify-between gap-4",
          graduated
            ? "border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30"
            : "border-primary-blue-100 bg-black/20",
        )}
      >
        <div className="flex items-center gap-4">
          <div
            className={cn(
              ...
              graduated
                ...
                : "bg-primary-blue-100 text-white/70",
            )}
          >
            {graduated ? "✓" : "–"}
          </div>
          <div>
            ...
            <p className="text-xs text-white/70 mt-0.5">
              ...
              &nbsp;·&nbsp; Communication: {commCount}/{commItems.length} (need
              4/5 — 80%)
            </p>
          </div>
        </div>
      ...
      <div className="space-y-8">
        {phases.map((phase) => {
          ...
          return (
            <div
              key={phase.id}
              className="rounded-[5px] border border-primary-blue-100 overflow-hidden"
            >
              {/* Header */}
              ...
              <div className="h-1 bg-black">
                <div
                  className={cn(
                    ...
                    pct === 100 ? "bg-emerald-500" : "bg-primary-blue-100",
                  )}
                  style={{ width: `${pct}%` }}
                />
              </div>
              ...
              <div className="divide-y divide-primary-blue-100">
                {phase.items.map((item) => (
                  <label
                    key={item.id}
                    htmlFor={item.id}
                    className="flex items-start gap-3 px-4 py-3.5 cursor-pointer hover:bg-primary-blue-100/20 transition-colors has-[button[data-state=checked]]:bg-primary-blue-100/10"
                  >
                    <Checkbox
                      id={item.id}
      ...
      <div className="mt-8">
        ...
        <div className="rounded-[5px] border border-primary-blue-100 divide-y">
          {[
            {
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-white/70 mt-0.5">{item.req}</p>
              </div>
            </div>
          ))}
        </div>
