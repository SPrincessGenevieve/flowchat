      ...
      <section className="mb-10">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {whatVaDoes.map((item) => (
            <Card
              key={item.title}
              className="shadow-none bg-black/20 border border-primary-blue-100/40"
            >
        ...
        <ol className="space-y-3">
          {dailySteps.map((step, idx) => (
            <li key={idx} className="flex items-start gap-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-blue-100 text-xs font-semibold text-white/70">
                {idx + 1}
              </span>
              <p className="text-sm text-white leading-relaxed pt-0.5">
                {step}
              </p>
            </li>
          ))}
      ...
      <section className="mb-10">
        ...
        <div className="space-y-3">
          {flaggedSteps.map((item) => (
            <div
              key={item.step}
              className="flex gap-4 rounded-lg border border-primary-blue-100 p-4"
            >
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary-blue-100 text-primary-foreground text-sm font-bold">
                {item.step}
              </div>
              <div>
      ...
      <section className="mb-10">
        ...
        <ul className="space-y-3">
          {clientHelps.map((item, idx) => (
            <li key={idx} className="flex items-start gap-3">
              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-primary-blue-100" />
